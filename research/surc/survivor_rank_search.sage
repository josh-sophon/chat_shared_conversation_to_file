...
# Universal class-57 point, already known to PARI on these fibers.
x0 = 36*(U-6)**2*(3*U-32)**2
y0 = 144*(U-6)*(3*U-32)*(26*U**4-771*U**3+9130*U**2-49344*U+106496)
P0 = E([x0,y0])

report = {
  'sage_version': str(SAGE_VERSION), 'U': int(U), 'mode': MODE,
  'parameter': int(PARAM), 'A': str(A), 'B': str(B),
  'curve': str(E), 'known_point': [str(P0[0]),str(P0[1])],
  'status': 'started'
}

def pj(P):
    return 'O' if P.is_zero() else [str(P[0]),str(P[1])]

def attempt(name, fn):
    try:
        value = fn()
        report[name] = {'status':'success','repr':repr(value)}
        return value
    except BaseException as exc:
        report[name] = {'status':'failure','type':type(exc).__name__,'message':str(exc),'traceback':traceback.format_exc()}
        return None
...
if gens is not None:
    report['gens'] = [pj(P) for P in gens]
    ...
    if len(gens) >= 2:
        ...
        if sat is not None:
            report['saturation_basis'] = [pj(P) for P in sat[0]]
            report['saturation_index'] = str(sat[1])
            report['saturation_regulator'] = str(sat[2])
            report['status'] = 'two_generators_found'
    else:
        report['status'] = 'one_generator_only'
else:
    report['status'] = 'no_generator_result'
